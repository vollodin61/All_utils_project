...
text = (f'Приветствую! Меня зовут Филатова Ирина, я тренер Лингвистики, соавтор одноименной книги)) {emoj(Emo.hugs)}\n\n'
...
def sender():
	...
	for i in ids_set:  # Перебор всех id в из списка
		with Client('my_account', my_id, my_hash) as app, open('text.txt', 'r', encoding='utf-8') as f:
			text = f.read()
			app.send_message(i, text=text_to_code(text))  # Отправляем сообщение

	# app = Client('my_account', api_id=my_id, api_hash=my_hash)
	# app.start()
	# app.send_message(
	# 	"me",
	# 	(
